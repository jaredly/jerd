...
//                     if (getTypeErrorOld(null, c.other, final) !== true) {
...
//     // make getTypeErrorOld indicate what kind of failure we're dealing with
...
//         if (getTypeErrorOld(null, constraint.other, one) !== true) {
//             throw new Error(
...
//         }
//     } else {
//         if (getTypeErrorOld(null, one, constraint.other) !== true) {
//             throw new Error(
...
export const assertFits = (
    ...
    if (getTypeErrorOld(env, t, expected) !== true) {
        throw new Error(
...
export const getTypeErrorOld = (
    ...
    if (found.type === 'var' && env != null) {
        if (typesEqual(found, expected)) {
            return true;
        }
        if (!env.local.tmpTypeVbls[found.sym.unique]) {
            throw new Error(
                `Explicit type variable ${found.sym.name}#${
                    found.sym.unique
                } can't unify with ${showType(env, expected)} - ${showLocation(
                    found.location,
                )} : ${showLocation(expected.location)}`,
            );
        }
        env.local.tmpTypeVbls[found.sym.unique].push({
            type: 'smaller-than',
            other: expected,
        });
    ...
    if (expected.type === 'var' && env != null) {
        if (!env.local.tmpTypeVbls[expected.sym.unique]) {
            throw new Error(
                `Unable to unify ${showType(env, found)} ${showLocation(
                    found.location,
                )} with type variable ${expected.sym.name}#${
                    expected.sym.unique
                } ${showLocation(expected.location)}`,
            );
        }
        env.local.tmpTypeVbls[expected.sym.unique].push({
            type: 'larger-than',
            other: found,
        });
        // TODO check if it's obviously broken
    ...
    switch (found.type) {
        case 'var':
            if (expected.type === 'var') {
                if (!symbolsEqual(found.sym, expected.sym)) {
                    return expected.sym;
                }
                return true;
            }
            ...
            return false;
        case 'ref':
            return typesEqual(found, expected);
        case 'lambda':
            if (expected.type !== 'lambda') {
                return false;
            }
            // unless there's optional arguments going on here, stay tuned?
            // I guess. maybe.
            if (expected.args.length !== found.args.length) {
                // console.log('arglen');
                return false;
            }
            const res = getTypeErrorOld(env, found.res, expected.res);
            if (res !== true) {
                console.log('resoff', found.res, expected.res);
                return res;
            }
            ...
            if (!effectsMatch(expected.effects, found.effects, false)) {
            ...
            for (let i = 0; i < found.args.length; i++) {
                ...
                if (arg !== true) {
                    ...
                    console.log(found.args[i], expected.args[i]);
                    return arg;
                }
            }
            return true;
