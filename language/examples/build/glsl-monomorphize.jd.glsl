...
const unnamed#test_main = (env#:0: GLSLEnv#451d5252, fragCoord#:1: vec2#builtin): vec4#builtin ={}> {
    ...
    const size#:2 = env#:0.resolution#451d5252#1.x#43802a16#0 /#builtin 20.0;
...
vec4 Vtest_main(
    ...
    float size = (env_0.resolution.x / 20.0);
    float size_3 = (size * 0.40);
    if (((t0 && (length((fragCoord_1 - (vec2(1.0, 1.0) * size))) < size_3)) || (t1 && (length(
        (fragCoord_1 - (vec2(2.0, 1.0) * size))
    ) < size_3)))) {
