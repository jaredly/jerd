...
bool within_004be9c0(float found_0, float expected_1, float margin_2) {
...
const unnamed#test_main = (env#:0: GLSLEnv#451d5252, fragCoord#:1: vec2#builtin): vec4#builtin ={}> {
    const t0#:4 = sqrt#builtin(4.0) ==#c41f7386#553b4b8e#0 2.0;
    const t1#:5 = within#004be9c0(found: sin#builtin(PI#builtin), expected: 0.0, margin: 0.0001);
    const t2#:6 = floatToInt#builtin(2.3) ==#9275f914#553b4b8e#0 2;
...
/* (
    ...
    const t0#:4: bool = sqrt(4) == 2;
    const t1#:5: bool = within#🛀🧱😆(sin(PI), 0, 0.0001);
    const t2#:6: bool = floatToInt(2.3) == 2;
...
vec4 Vtest_main(GLSLEnv_451d5252 env_0, vec2 fragCoord_1) {
    bool t0 = (sqrt(4.0) == 2.0);
    bool t1 = within_004be9c0(sin(PI), 0.0, 0.00010);
    bool t2 = (floatToInt(2.30) == 2);
